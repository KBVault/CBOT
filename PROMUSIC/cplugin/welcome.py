...
def welcomepic(pic, user, chatname, id, uname, brightness_factor=1.3):
    background = Image.open("PROMUSIC/assets/wel2.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp, brightness_factor=brightness_factor)
    pfp = pfp.resize((892, 880))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype("PROMUSIC/assets/font.ttf", size=95)
    welcome_font = ImageFont.truetype("PROMUSIC/assets/font.ttf", size=45)
...
async def greet_new_member(
    ...
    if member.new_chat_member and not member.old_chat_member:
        ...
        try:
            ...
            deep_link = f"{user.id}"
            add_link = f"https://t.me/NezukoProBot?startgroup=true"
